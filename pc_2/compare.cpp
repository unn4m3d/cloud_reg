...
CorrData loadCorrs(std::string name)
{
    CorrData set;
    ...
    for(auto& elem : j)
    {
        ...
        if(elem.is_array())
        {
            for(auto& pt : elem[0])
            {
                corr.first.push_back(pt.get<PtIndex>());
                set.all_pts.first.push_back(pt.get<PtIndex>());
            }
            for(auto& pt : elem[1])
            {
                corr.second.push_back(pt.get<PtIndex>());
                set.all_pts.second.push_back(pt.get<PtIndex>());
            }
        }
        set.set.push_back(corr);
    }
...
void check(AlignResult& result, const PointCloudT::Ptr& scene, const PointCloudT::Ptr& aligned, const CorrData& corrs)
{
    result.align_right = std::all_of(corrs.set.cbegin(), corrs.set.cend(), 
        [&](const auto& corr)
        {
            return std::all_of(corr.first.cbegin(), corr.first.cend(), 
                [&](const auto& ia)
                {
                    const auto& a = scene->at(ia);
                    // One of points in correspondence is nearest to given point of all points
                    const auto& nearest = *std::min_element(corrs.all_pts.second.cbegin(), corrs.all_pts.second.cend(),
                        [&](const auto& ib, const auto& ic)
                        {
                            const auto& b = scene->at(ib), &c = scene->at(ic);
                            return 
                                Eigen::Vector3f(a.x - b.x, a.y - b.y, a.z - b.z).norm() < 
                                Eigen::Vector3f(a.x - c.x, a.y - c.y, a.z - c.z).norm();
                        }
                    );

                    return std::find(corr.second.cbegin(), corr.second.cend(), nearest) != corr.second.cend();
                }
            );
        }
    );
...
AlignResult align(const PointCloudT::Ptr& object, const PointCloudT::Ptr& scene, const PointCloudT::Ptr& aligned)
{
  ...
  {
    return {initial, align.getFinalTransformation(), align.getFitnessScore(), true, false};
  }
  else
  {
    return {initial, Eigen::Matrix4f::Identity(), 0.0, false, false};
  }
}
...
main (int argc, char **argv)
{
  ...
  auto cset = loadCorrs(std::string{ argv[3]}); 
  ...
  for(int i = 0; i < N; ++i)
  {
    ...
    threads.emplace_back([&, i](){
      ...
      check(results[i], scene_copy, aligned[i], cset);
    });
  }
  ...
  int index = -1;
  double max_score = 100500;

  for(int i = 0; i < results.size(); ++i)
  {
    if(results[i].ok)
    {
      if(results[i].score > max_score && results[i].align_right)
      {
        index = i;

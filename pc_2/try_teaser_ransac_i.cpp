...
int main()
{
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ> ("../test/extracted.pcd", *object) < 0 ||
        pcl::io::loadPCDFile<pcl::PointXYZ> ("../test/model.pcd", *scene) < 0)
    {
    ...
    pcl::transformPointCloud(*object_d, *aligned, getTransform(solution).matrix());
    ...
    visu.registerKeyboardCallback([&](const pcl::visualization::KeyboardEvent& kb)
    {
        if(kb.keyDown()) return;

        auto pt = randomRotationMatrix();
        auto matrix = pt.matrix();
        pcl::transformPointCloud(*aligned, *aligned, matrix);

        auto solution = teaserStep(aligned, scene_d, params);

        pcl::transformPointCloud(*aligned, *aligned, getTransform(solution).matrix());
        visu.updatePointCloud(aligned, "object_aligned");
    });

...
main (int argc, char **argv)
{
  ...
  if (argc < 4)
  {
    pcl::console::print_error ("Syntax is: %s scene.obj object.obj transform.tr crop.tr\n", argv[0]);
    return (-1);
  }
  ...
  auto crop_mat = clouds::parse_matrix<3,3,float>(crop);
...
pcl::console::print_highlight ("Downsampling...\n");
  ...
  centroid /= scene->points.size();
   ...
   /* pcl::search::Search<PointNT>::Ptr tree;
    ...
    icp.align (*object_a);
    if(icp.hasConverged())
        visu.addPointCloud (object_a, ColorHandlerT(object_b, 0.0, 255.0, 255.0), "oa");
    else 
        std::cout << "Oops!" << std::endl;

    std::cout << "ICP Transformation: " << std::endl;
    std::cout << icp.getFinalTransformation().inverse() << std::endl;
    std::cout << "Final transformation: " << std::endl;
    std::cout << ( mat * icp.getFinalTransformation().inverse()) << std::endl;
